...
correctdir = "/Volumes/sd17B002/Microscopy-Dragonfly/Image-Analysis/Flat-Field-Correction/";
...
for (j=0; j<lns.length; j++){
	...
	if (lns[j] == "[Imaging Modes In Protocol]"){
		ctrl=1;
		}
		
	//extract used laser lines
...
Dialog.addChoice("Extract channel (excitation wavelength in nm):", wavelengths);
Dialog.show();
extract = Dialog.getChoice();
print(logfile, "Extracting the " + extract + " nm channel for stitching");

//get the index of the channel to be used for stitching (usually the last for DAPI)
for (n=0; n<wavelengths.length; n++){
	if (wavelengths[n]==extract){
		extract_index = n+1;
		}
	}
...
outdirmaxch = outdirmax + "/_" + extract + "/";
...
for(i=0; i<filelist.length; i++) {
	if (endsWith(filelist[i], ".ims")){
		...
		check2 = outdirmaxch+extract+"nm_MAX_"+replace(filelist[i], ".ims", ".tif");
		check3 = outdirmaxchcorr+extract+"nm_MAX_ffcorr_"+replace(filelist[i], ".ims", ".tif");
		if (!File.exists(check2)){
			print("Converting file "+(i+1)+"/"+filelist.length+" ("+filelist[i]+")");
			if (File.exists(check)){
				open(check);
				//print(logfile, "Extracting "+extract+"nm_MAX_"+replace(filelist[i], ".ims", ".tif"));
				} else {
				...
				run("Duplicate...", "duplicate channels=extract_index-extract_index");
				setMinAndMax(0, 65535);
				saveAs("Tiff", outdirmaxch+extract+"nm_MAX_"+filelist[i]);
				rename("RawDAPI");
				setMinAndMax(0, 65535);
			} else {
				if (!File.exists(check3)){
					open(check2);
					run("Duplicate...", "title=RawDAPI");
					}
				}
		if (extract == 405 && !File.exists(check3)){
			print("Correcting flatfield for 405 nm channel for "+filelist[i]);
			File.makeDirectory(outdirmaxchcorr);
			// calculate the corrected Image (RawImage-Dark) x Gain
			open(correctdir+gain);
			rename("gain");
			setMinAndMax(0, 65535);
			open(correctdir+"2022-03-10-dark-counts.tif");
			rename("dark");
			setMinAndMax(0, 65535);
			...
			run("16-bit");
			setMinAndMax(0, 65535);
			saveAs("Tiff", outdirmaxchcorr+extract+"nm_MAX_ffcorr_"+filelist[i]);
			}
		close("*");
		} else {
...
if (File.exists(outfile)){
	print(logfile, "-----Warning------: the tile configuration file already exists. Renaming the old one before writing the new one.");
	old_outfile = replace(outfile, ".txt", "_old.txt");
	File.rename(outfile, old_outfile);
	} 
if (File.exists(outfile2)){
	print(logfile, "-----Warning------: the tile configuration file already exists. Renaming the old one before writing the new one.");
	old_outfile = replace(outfile2, ".txt", "_old.txt");
	File.rename(outfile2, old_outfile);
	} 
for (k=0; k<lns.length; k++){
	if (startsWith(lns[k], extract)){
		lns[k] = replace(lns[k], extract + "nm_MAX", "MAX");
		line = split(lns[k], ";");
		coord = split(line[2], ",");
		x = replace(coord[0], "\\(", "");
		y = replace(coord[1], "\\)", "");
		...
		if (abs(x - x_init) + abs(y - y_init) > 400){
		...
		lns[k] = replace(lns[k], "MAX", extract+"nm_MAX_ffcorr");
...
Stack.getDimensions(width, height, channels, slices, frames);
run("Split Channels");
filenm = replace(filenm, extract + "nm_MAX", "MAX");
...
setMinAndMax(0, 65535);
saveAs("Tiff", fused+filenm+extract+"nm_corr.tif");
